import type { AuthSession, DemoPerson, Profile, ProgressUpdate, RecommendationResult, Role } from './types'
...
export const api = {
  ...
  action: async (id: string, event_id: string, action: 'complete' | 'enroll' | 'not_now') => { if (mock) { await wait(350); return progress } return request<ProgressUpdate>(`/api/employees/${id}/activities`, { method: 'POST', body: JSON.stringify({ event_id, action }) }) },
}
